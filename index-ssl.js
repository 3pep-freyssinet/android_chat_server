			...
			*/
			...
			async function displayContent() {
				try{
					...
					//for (let i = 0; i <= y.length - 1; i++){ 
					  ...
					  //console.log('query2 = ' + query2 + ' user = ' + users[5]);
			...
			async function app() {
				var row2 = await displayContent().then((result) =>{  //wait the return of 'new Promise' inside 'displayContent'. 
					...
					for(let i = 0; i <= results1.rows.length - 1; i++){
						var temp ={};
...
var people0		= {};			//store the connected pair : current user and selected user like people0[current]=selected. It is an object with keys accessed like : Object.keys(people0)
...
io.use((socket, next) => {
  if (socket.handshake.query && socket.handshake.query.token) {
	...
	jwt.verify(socket.handshake.query.token, 'secret', (err, decoded) => {
		...
		console.log('decoded keys = ' + Object.keys(decoded)); //emailId, emailIdOTP, phoneNo
...
io.on('connection', async (socket) => {
  ...
  console.log("socket.handshake.query = " + socket.handshake.query.token); //['loginId']);
  ...
  if (clients != null) {
    for (const clientId of clients) {
      ...
      console.log("client.decoded.emailId = " + client.decoded.emailId)
    }
  }
})
/////////////////////////////////////////////////////////////////////////////////////////////////////
//detect the user first connection
io.on('connection_', (socket) => {
	...
	function timeout(){
		...
		logoffTimer = setTimeout(function(){//this function in executed after the delay
			...
			// wait a response from client. set a timount
				...
				// If the client does not respond, send him a disconnection notification
				//socket.emit("inactif", { reason: "inactivity after connection", socketId : socket.id });
				// set a timount
   ...
   //timeout();
	...
	socket.on('disconnect', ()=>{
		console.log("socket.id = " + socket.id + " " + people[socket.id] + " has disconnected");
		...
		console.log(user + " in people0 apres delete " + (user in people0));
		...
		var nickname;
		while (!	key.done) {
			 console.log("map = " + myMap.get(key.value) + " : " + key.value); // key : value
		...
		//if(myMap.size == 0)socket.disconnect(0); //socket.destroy();
	...
	socket.on('join', (chatUserInfos) => {
		timeout();
		console.log("'join' chat_user_infos chatUserInfos socketid = "+socket.id);
		...
		var imageProfile 		= typeof obj.Profile == 'undefined' ? null : obj.Profile; 
		...
		var imageProfileUri 	= obj.ProfileUri;
		...
		*/
		...
		if(imageProfileUri != null){
			var query = "INSERT INTO imageprofileuri (nickname, uri, date)" 	+
			" VALUES($1, $2, $3)"										    	+    					
			" ON CONFLICT (uri) DO"												+ 
			...
			//Put only necessary values, otherwise error
			pool.query(query,[userNickname, imageProfileUri, Date.now()], async(error, results) =>{
				const promise = new Promise((resolve, reject) => {
					 resolve(results); 
				});
				...
				promise.then((value) => {	// value et result la même chose
				  ...
				  //callback(res);
					...
					console.log("promise 'INSERT into imageprofileuri' error : " + error.message);
					console.log("promise 'INSERT into imageprofileuri' error : " + error.stack);
					//console.error(error);
				});
			});
		}
        ...
        console.log(socket.username + " : has joined the chat, id = " + socket.id +" , image profile = " +/*imageProfile*/ + " , time = " + connectionTime + ", last = " + lastConnectionTime);
		...
		console.log("****************** userjoinedthechat = " + userNickname);
	...
	socket.on('get_image_profile_uri', (name, callback) => {
		...
		pool.query(query,[name], async(error, results) =>{
			...
			promise.then((value) => {	// value et result la même chose
			  ...
			  //console.log("promise 'get_image_profile_uri'  results.rowCount = " + results.rowCount); //JSON.stringify(results.rowCount));
	...
	*/
	...
	socket.on('get_users', (nicknames) => {	//'nicknames' =array
		...
		var query = "SELECT nickname, encode(imageprofile, 'escape') as imageprofile, status, connected, lastconnected, disconnected, blacklistauthor, notseenmessages, connectedwith FROM users WHERE nickname = any ($1)" ;
	...
	*/
	...
	socket.on('messagedetection', (message, callback) => { //message is json object, callback is used to send boolean for seen or unseen message
	   ...
	   console.log("messagedetection, fromNickname = "+myMap.get(socket.id)+" fromId = "+socket.id);
        ...
        */
		...
		if((people[toNicknameId] != null)){
			//the user who would received the message is connected.
			//console.log("the user who would received the message is connected");
			socket.to(toNicknameId).emit('message_detection_back', json);
		}else{
			/*
			//the user who would received the message is not connected. increment the 'notSeenMessages' field of sender
   ...
   socket.on('get_all_not_seen_messages', function(nickname){	
	...
	pool.connect(async (err, connection, release) => {
		...
		' messages.fromnickname as "fromNickname", messages.tonickname as "toNickname",  messages.message, messages.time, encode(messages.extra, \'escape\') AS "extra" , '  + 
		' messages.extraname as "extraName", messages.ref, messages.mime as "mimeType", messages.seen, messages.deletedto as "deletedTo", messages.deletedfrom as "deletedFrom" ' +
		' FROM messages ' +
		' WHERE messages.tonickname = $1' + 
		' AND messages.seen = cast(0 as bit(1))';
		...
		' count(messages.fromnickname) AS "nbMessages", users.nickname, encode(users.imageprofile, \'escape\') AS "imageProfile"' + //, users.status " + //, users.connected, users.lastconnected, users.disconnected " + 
		' FROM messages ' +
		' LEFT JOIN users ' +
		' ON messages.fromnickname = users.nickname ' + 
		' WHERE messages.tonickname = $1' + 
		...
		var query3 = 'SELECT ' + 
		 ' fromnickname, tonickname, MAX(time) AS time' +    // la fonction MAX(time) prend la plus grande valeur de time pour distinguer les valeur de time à garder, car il peut y avoir plusieurs valeurs.
		 ' FROM messages  ' + 
		 ' WHERE messages.tonickname = $1' + 
		 ' AND messages.seen = cast(0 as bit(1)) ' + 
		 ' group by fromnickname, tonickname';
		...
		if (notSeenMessages.rowCount > 0) {
			let notSeenMessagesAuthor = await executeQuery(pool, query2, [nickname]); //the users who send the not seen messages
			let lastContacts          = await executeQuery_(pool, query3, [nickname], release); //last users who send messages 
			//let users 		      = await executeQuery(pool, query4, [nickname]);
			if(notSeenMessages.rowCount == 0)console.log('*************error : notSeenMessages is null and  messages is not null. it is not accepted ********');
			result["notSeenMessages"] 		= notSeenMessages.rows;
			result["notSeenMessagesAuthor"] = notSeenMessagesAuthor.rows;
			result["lastContacts"]          = lastContacts.rows;
			...
			*/
			...
			result["notSeenMessagesAuthor"] = null;
		...
		pool.query(query, [reference], async(error, results) =>{
			...
			promise.then((value) => {
			  ...
			  var json = {"Ref" : reference, "Size" : datab.length, "Data" : datab.toString()};	//json object
				...
				console.log("promise SELECT filedata FROM images "+error.message); 
			});
		});
	});		

...
io.on('connection', (socket) => {
	...
	socket.on('monitor_users', () =>{
		console.log("*********************received monitor_users ***********************************************************");
		state.socket = socket;
		
		//emit current 'myMap' 
		const current_users = listeners.mapWatch();
		if(state.socket != null)state.socket.emit('monitor_users_back', current_users);
		
		//listeners.monitor_users();
	});
   ...
   */
	...
	*/
	...
	socket.on('disconnect', ()=>{
		listeners.disconnect(pool, socket);
	});
	...
	socket.on('join', (chatUserInfos) => {
		const length = (chatUserInfos.Profile != null) ? chatUserInfos.Profile.length : 0;
		console.log("????????????????????????????????? join : chatUserInfos.Profile.length = " + length);
		listeners.join(pool, socket, chatUserInfos);
	...
	socket.on("save_image_profile_uri", (profile, callback) =>{
		listeners.save_image_profile_uri(pool, socket, profile, callback);
	});
	...
	socket.on('get_all_users', () => { 
		listeners.get_all_users(pool, socket);
	});

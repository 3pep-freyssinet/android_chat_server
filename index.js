   ...
   */
	...
	socket.on('join', (chatUserInfos) => {
		...
		*/
		...
		if(imageProfileUri != null){
			...
			//Put only necessary values, otherwise error is raised.
		...
		//store the user in map if it not exists
	...
	*/
	...
	socket.on('messagedetection', (message, callback) => { //message is json object, callback is used to send boolean for seen or unseen message
	   ...
	   //console.log('obj.ref = %s, obj1.ref = %s', obj.ref, obj1.ref);
		...
		while (!	key.done) {
			 ...
			 if(myMap.get(key.value) == toNickname_){
				toNicknameId = key.value;
				break;
			 }
			 key = keys.next();		 //next key
		}
		console.log("message detectionEnd Map-----------------------------------------------");
		console.log("**apres map **********toNickname = " + toNickname_ + " toNicknameId = " + toNicknameId );
		
		for(var key_ in people) {
			//console.log("join : people keys : Key: " + key + " Value: " + people[key]);
			if(people[key_] == toNickname_)toNicknameId = key_
		}
		console.log("**people **********toNickname = " + toNickname_ + " toNicknameId = " + toNicknameId );
        ...
        */
		...
		console.log("message_detection_back, the user who would receive the message, his name = '" + people[toNicknameId] + "' his id = " + toNicknameId);
		
		//socket.emit('message_detection_back', json); 
		//io.emit('message_detection_back', json);
		if((people[toNicknameId])){ // or if(people.hasOwnProperty(toNicknameId)){
   ...
   socket.on('get_all_not_seen_messages', function(nickname){	
	...
	pool.connect(async (err, connection, release) => {
		...
		//all not seen messages sent by 'fromnickname'
		...
		//all not seen messages sent by 'fromnickname' after disconnect
		...
		//count the not seen messages sent by 'fromnickname'
		...
		*/
		...
		if(notSeenMessages.rowCount == 0) {
			//release(); //there is error here "Error: Release called on client which has already been released to the pool."
		}

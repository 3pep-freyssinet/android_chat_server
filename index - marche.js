...
authenticate = function (req, res, done) {
    ...
    if (credentials) {
        ...
        db.all(query, function (err, rows) { //sqlite command
...
getChannels = function (req, res) {
    authenticate(req, res, function () {
        ...
        db.each(query, function (err, row) {//sqlite command
...
socket.on('messagedetection', (toNicknameId, message) => { //message is string object
	   ...
	   var query  = "INSERT into messages (ref, fromnickname, tonickname, message, time, extra, extraname) VALUES ($1, $2, $3, $4, $5, $6, $7)"; //The id is automatically added if it is auto increment
		...
		pool.query(query, [ref_, fromNickname_, toNickname_, messageContent, time_, extra_, extraName_], (error, results) => {  //INSERT //$1=data, $2=5 (row id =5).
			...
			console.log('*************************************************************************************')
			console.log('insert results.rows : %s', results.rows);
			console.log('*************************************************************************************')
		});
...
socket.on('uploadFileChuncks', function (data){
        ...
        if(Files[Name]['Downloaded'] == Files[Name]['FileSize']) //If File is Fully Uploaded
        {
			...
			//var query = "INSERT INTO images (name, filedata, owner,  dateposted) VALUES($1, $2, $3, $4)";
				...
				//console.log('name = %s, size = %d, data = %s', Name, dataa.length, dataa); 
				...
				pool.query(query, [ref, Name, owner, dataa, time], (error, results) => {  //INSERT //$1=data, $2=5 (row id =5).
					if (error) {
						console.log('*** error  update ****'+error);
					}
					console.log('*************************************************************************************')
					console.log('update results.rows : %s', results.rows);
					console.log('*************************************************************************************')
				});
			...
			fs.stat(fileName, function(error, stats) {
				...
				fs.open(fileName, "r", function(error, fd) {
					fs.read(fd, buffer, 0, buffer.length, null, function(error, bytesRead, buffer) {
						...
						pool.query(query, [reference], (error, results) => {
							if (error) {
								throw error
							};
							var datab = results.rows[0].filedata;
							...
							//JSON.stringify({ 'Name' : Name, 'Data' : datab});
							...
							var json = {"Data" : datab.toString()};
							//const obj = JSON.parse(json);	
							console.log("*******************************************************************************");
							console.log("*** download_chunks ***\n");
